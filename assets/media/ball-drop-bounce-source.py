#!/usr/bin/env python3
"""Deterministic 1D BallDrop animation and synchronized height-time trace.
...
if SCRIPT_PATH.parent.name == "media" and SCRIPT_PATH.parent.parent.name == "assets":
...
SOURCE_NAME = OUT_DIR / "ball-drop-bounce-source.py"
...
def load_font(size: int) -> ImageFont.ImageFont:
    ...
    for candidate in candidates:
        try:
            ...
            pass
    raise RuntimeError("No website-compatible sans-serif font could be loaded")


FONT_TICK = load_font(18)
FONT_LABEL = load_font(18)
FONT_MARKER = load_font(18)
...
def draw_axes(draw: ImageDraw.ImageDraw, frame: Image.Image) -> None:
    # Axes
    draw.line(coords([PLOT_X0, PLOT_Y1, PLOT_X1, PLOT_Y1]), fill=TEXT, width=S(1.1))
    draw.line(coords([PLOT_X0, PLOT_Y0, PLOT_X0, PLOT_Y1]), fill=TEXT, width=S(1.1))

    # x ticks and labels
    for tick in [0, 1, 2, 3, 4]:
        x, _ = plot_xy(float(tick), 0.0)
        draw.line(coords([x, PLOT_Y1, x, PLOT_Y1 + 5]), fill=TEXT, width=S(0.9))
        label = str(tick)
        bbox = draw.textbbox((0, 0), label, font=FONT_TICK)
        draw.text((S(x) - (bbox[2] - bbox[0]) // 2, S(PLOT_Y1 + 16)), label, fill=TEXT, font=FONT_TICK)

    # y ticks and labels
    for tick, label in [(0.0, "0"), (0.5, "0.5"), (1.0, "1.0")]:
        _, y = plot_xy(0.0, tick)
        draw.line(coords([PLOT_X0 - 5, y, PLOT_X0, y]), fill=TEXT, width=S(0.9))
        bbox = draw.textbbox((0, 0), label, font=FONT_TICK)
        draw.text((S(PLOT_X0 - 18) - (bbox[2] - bbox[0]), S(y) - (bbox[3] - bbox[1]) // 2), label, fill=TEXT, font=FONT_TICK)

    # Axis labels
    x_label = "time"
    bbox = draw.textbbox((0, 0), x_label, font=FONT_LABEL)
    draw.text((S((PLOT_X0 + PLOT_X1) / 2) - (bbox[2] - bbox[0]) // 2, S(552)), x_label, fill=TEXT, font=FONT_LABEL)
    draw_rotated_text(frame, (S(405), S(236)), "ball height", FONT_LABEL, TEXT)


def draw_simulator(draw: ImageDraw.ImageDraw, current_y: float) -> None:
    # Ground line, no labels.
    draw.line(coords([80, SIM_GROUND_Y, 300, SIM_GROUND_Y]), fill=TEXT, width=S(1.2))
...
def render_frame(current_t: float) -> Image.Image:
    ...
    draw.line(coords([DIVIDER_X, 54, DIVIDER_X, 546]), fill=LINE, width=S(0.9))
...
def save_gif(frames: Sequence[Image.Image]) -> None:
    # Adaptive palette plus frame-layer optimization keeps the 4800x1800 GIF lighter.
...
def write_source_snapshot() -> None:
    if SCRIPT_PATH.resolve() == SOURCE_NAME.resolve():
        return
    shutil.copy2(SCRIPT_PATH, SOURCE_NAME)
...
def main() -> None:
    ...
    for path in [GIF_NAME, MP4_NAME, WEBM_NAME, SOURCE_NAME]:
